...
- [CSS 面试知识点总结](#css-面试知识点总结)
  - [目录](#目录)
    ...
    - [6.CSS 选择器优先级权重？](#6css-选择器优先级权重)
    - [7.关于a标签的伪类状态?](#7关于a标签的伪类状态)
    ...
    - [45.什么是响应式设计？响应式设计的基本原理是什么？如何兼容低版本的 IE？](#45什么是响应式设计响应式设计的基本原理是什么如何兼容低版本的-ie)
...
简单的方式：
	header 和 footer 固定了高度，中间的 main 区域填充了剩余的空间，实现了品字布局。你可以根据实际需求调整高度和样式。
  ```
<!--html-->
<div class="container">
...
width:100%会使元素box(border-box)的宽度等于父元素的content-box的宽度。
width:auto会使元素撑满整个父元素，margin、border、padding、content区域会自动分配水平空间(也就是外边距的宽度也加在子元素的宽度上)。
...
```
详细资料可以参考：
[CSS面试常考题：清除浮动](https://juejin.cn/post/7295938830838251555?searchId=202404201840466EA0020CC1AC39F4379C)
