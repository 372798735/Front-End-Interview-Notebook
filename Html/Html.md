...
#### 21. CSS 如何阻塞文档解析？（浏览器解析过程） 
   ```
    1、放在<head>中的CSS: 当浏览器遇到放置在<head>中的CSS标签时，它会阻塞文档的渲染和解析，直到CSS文件完全下载并解析完毕。
    这意味着浏览器会等待CSS文件加载完成，然后才会继续解析HTML文件。

    2、内联CSS: 如果CSS是内联到HTML文件中的，即在<style>标签内或者在HTML元素的style属性中，浏览器会在解析到CSS代码时暂停
    文档解析，并开始解析和应用CSS。这会延迟HTML的渲染，尤其是在CSS代码很长或复杂时。

    3、使用 @import 导入的CSS: 如果CSS文件使用 @import 指令在其他CSS文件中导入，浏览器会在解析到 @import 语句时暂停文档的
    解析，直到导入的CSS文件加载和解析完毕。这可能会导致额外的网络请求和较长的页面加载时间。

    4、渲染阻塞的CSS属性：有些CSS属性会触发渲染阻塞，例如 font-display: swap。这些属性会阻止页面渲染，直到它们所影响的资源
    加载完成。
   ```
